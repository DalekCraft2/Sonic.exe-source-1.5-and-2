...
class OptionCata extends FlxSprite
{
	...
	public function new(x:Float, y:Float, _title:String, _options:Array<Option>, middleType:Bool = false)
	{
		...
		middle = middleType;
		if (!middleType)
			makeGraphic(Math.floor(1180 / 5), 64, FlxColor.BLACK); // Width of black frame divided by number of visible categories
		alpha = 0.4;
	...
	{
		makeGraphic(Math.floor(1180 / 5), 64, color);
	}
}
...
class OptionsMenu extends FlxSubState
{
	...
	override function create()
	{
		options = [
			...
			new OptionCata(50 + Math.floor(1180 / 5), 40, "Gameplay", [
				...
				new DFJKOption("Edit your keybindings"),
				new Judgement("Create a custom judgement preset"),
				new CustomizeGameplay("Drag and drop gameplay modules to your prefered positions!")
			]),
			new OptionCata(50 + Math.floor(2 * 1180 / 5), 40, "Appearance", [
			...
			new OptionCata(50 + Math.floor(3 * 1180 / 5), 40, "Misc", [
			...
			new OptionCata(50 + Math.floor(4 * 1180 / 5), 40, "Saves", [
		...
		{
			if (i >= options.length - 2) // Size of options array without keybind and judgement menus
				continue;
			var cat = options[i];
			add(cat);
			add(cat.titleObject);
			...
			else
			{
				...
				if (selectedOption.acceptType || !selectedOption.acceptType)
				{
					...
					if (escape)
					{
						...
						if (selectedCatIndex >= options.length - 2) // Size of options array without keybind and judgement menus
							selectedCatIndex = 0;
