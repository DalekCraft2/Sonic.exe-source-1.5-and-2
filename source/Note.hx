...
class Note extends FlxSprite
{
			...
			?beat:Float = 0, ?noteType:Int = 0)
	{
		...
		quantityColor = isRing ? quantityColor5k : quantityColor;
		arrowAngles = isRing ? arrowAngles5k : arrowAngles;
		...
		if (inCharter)
		{
			...
			var fuckingSussy = Paths.getSparrowAtlas('noteskins/Arrows-static');
			...
			{
				case 3:
					frames = Paths.getSparrowAtlas('noteskins/Arrows-phantom');
					animSuffix = ' withered';
				case 2:
					frames = Paths.getSparrowAtlas('noteskins/Arrows-static');
					animSuffix = ' static';
				default:
					frames = PlayState.noteskinSprite;
					animSuffix = ' alone';
			...
			{
				animation.addByPrefix(dataColor[i] + 'Scroll', dataColor[i] + animSuffix); // Normal
			...
			switch (noteTypeCheck)
			{
				case 'pixel':
					...
					for (i in 0...dataColor.length)
					{
						if (noteType == 2)
						{
							animation.add(dataColor[i] + 'Scroll', [i + 20]); // Normal notes
						}
						else
						{
							animation.add(dataColor[i] + 'Scroll', [i + 4]); // Normal notes
						}
						animation.add(dataColor[i] + 'hold', [i]); // Hold
					...
					updateHitbox();
				case 'majin':
					frames = Paths.getSparrowAtlas('noteskins/Arrows-majin');
					var fuckingSussy = Paths.getSparrowAtlas('noteskins/Arrows-majin');
					for (amogus in fuckingSussy.frames)
					{
						this.frames.pushFrame(amogus);
					}
					...
					for (i in 0...dataColor.length)
					{
						animation.addByPrefix(dataColor[i] + 'Scroll', dataColor[i] + ' alone'); // Normal
						animation.addByPrefix(dataColor[i] + 'hold', dataColor[i] + ' hold'); // Hold
						animation.addByPrefix(dataColor[i] + 'holdend', dataColor[i] + ' tail'); // Tails
					}
