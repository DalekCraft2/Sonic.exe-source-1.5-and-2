...
class ChartingState extends MusicBeatState
{
	...
	**/
		...
		for (i in array)
		{
			...
			for (ii in _song.notes)
			{
				if (ii.startTime <= strum && ii.endTime > strum)
				{
					...
					var note:Note = new Note(strum, Math.floor(i[1] % keyArray[fucku]), null, false, true, i[3], i[4], i[5]);
	...
	function swapSection(secit:SwagSection)
	{
		...
		for (i in shownNotes)
		{
			for (ii in secit.sectionNotes)
				if (i.strumTime == ii[0] && i.noteData == ii[1] % keyArray[fucku])
				{
	...
	function updateGrid():Void
	{
		...
		for (section in _song.notes)
		{
			for (i in section.sectionNotes)
			{
				...
				var note:Note = new Note(daStrumTime, daNoteInfo % keyArray[fucku], null, false, true, isAlt, daBeat, daType);
	...
	private function addNote(?n:Note):Void
	{
		...
		if (n == null)
		{
			var note:Note = new Note(noteStrum, noteData % keyArray[fucku], null, false, true, TimingStruct.getBeatFromTime(noteStrum));
		...
		{
			var note:Note = new Note(n.strumTime, n.noteData % keyArray[fucku], null, false, true, n.isAlt, TimingStruct.getBeatFromTime(n.strumTime),
				n.noteType);
